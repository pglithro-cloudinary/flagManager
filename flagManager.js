javascript: (() => {
    ...
    if (titleH1.indexOf('Edit Cloud - ') >= 0) {
        cloudName = titleH1.substring(titleH1.indexOf('Edit Cloud - ') +
            13);
    } else {
        cloudName = "Undefined";
    }
    ...
    style.type = 'text/css';
    style.innerHTML =
        '.highlightFlagOff { border: 2px solid red;padding:1px;padding-top:3px;margin-top:2px;margin-bottom:1px;border-radius:5px;display:inline-block } .highlightFlagOn { border: 2px solid green; padding:1px;padding-top:3px;margin-right:2px;border-radius:5px;display:inline-block } .enum-multi { margin:3px 0;} table {border-collapse: collapse;margin: 20px 0} table td { padding: 5px;} table thead th { background-color: #54585d;color: #ffffff;font-weight: bold;font-size: 13px;border: 1px solid #54585d;} table tbody td {color: #636363;border: 1px solid #dddfe1;} table tbody tr { background-color: #f9fafb;} table tbody tr:nth-child(odd) {background-color: #ffffff;}';
    ...
    function saveFlags() {
        ...
        setCookie("PG-" + cloudName + "@@" + timeNow, cookieData, 1000);
        /* Add the new one to the list */
        savedListArray.push(cloudName + "@@" + timeNow);
    ...
    function buildContents() {
        ...
        newDiv +=
            "<h1 style='margin-bottom:40px'>Patrick's Flag Manager</h1>";
        if (cloudName != "Undefined") {
            newDiv +=
                "<div id='saveButton'><button type='button'>Save Current Config</button></div>";
    ...
    function compareClouds(comparedCloud) {
        var changesPending = document.querySelectorAll('.highlightFlagOff,.highlightFlagOn');
        ...
        savedListString = getCookie("PG-Saved-List");
        savedListArray = savedListString.split("|").filter(Boolean);
        comparisonCheckedFlags = getCookie("PG-" + savedListArray[
            comparedCloud]);
        comparisonCheckedFlagsArray = comparisonCheckedFlags.split("|");
        var changeText = "";
        var currentCheckedFlagsArray = readCurrentFlags()[1];
        ...
        for (let i = 0; i < toBeChecked.length; i++) {
            ...
            flagHolder.className = "highlightFlagOn";
        }
        ...
        for (let i = 0; i < toBeUnchecked.length; i++) {
            ...
            flagHolder.className = "highlightFlagOff";
        }

        document.getElementById("customer_admin_log").value =
            changeText;
        document.getElementsByName("commit")[0].value = "Save with copied flags!";
        toggleWindow();
        alert(changeText);
    }
